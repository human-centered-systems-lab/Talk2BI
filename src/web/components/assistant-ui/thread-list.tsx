...
const ThreadListItem: FC = () => {
  ...
  const isActive = remoteId && pathname === `/chat/${remoteId}`;
  ...
  return (
    <div
      className={`aui-thread-list-item group flex h-9 items-center gap-2 rounded-lg transition-colors focus-visible:outline-none ${isActive ? 'bg-muted' : 'hover:bg-muted focus-visible:bg-muted data-active:bg-muted'}`}
      onClick={handleClick}
    >
      <ThreadListItemPrimitive.Trigger 
        className={`aui-thread-list-item-trigger flex h-full min-w-0 flex-1 items-center px-3 text-start text-sm cursor-pointer ${isActive ? 'text-primary font-medium' : ''}`}
