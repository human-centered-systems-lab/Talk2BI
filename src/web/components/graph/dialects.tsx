...
export function SnowflakeBrandIcon({ className }: { className?: string }) {
  return (
    <svg
      role="img"
...
export function DatabricksBrandIcon({ className }: { className?: string }) {
  return (
    <svg
      role="img"
...
/** Every warehouse the Add Dataset dialog can configure. */
export const dialects = [
  { name: "Snowflake", icon: SnowflakeBrandIcon },
  { name: "Databricks", icon: DatabricksBrandIcon },
] as const;

export function DatasetDialectIcon({
  dialect,
  className,
}: {
  dialect: string;
  ...
  const match = dialects.find(
    (item) => item.name.toLowerCase() === dialect.toLowerCase(),
  );
  ...
  return <DatabaseIcon aria-label={dialect} className={className} />;
}
